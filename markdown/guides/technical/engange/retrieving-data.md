...
This guide shows you how to retrieve data from the store after [fetching and processing the data](data-fetching-and-processing.md). Because the loyalty points we introduced in the [Loyalty Points Tutorial](../../../tutorials/starter/loyalty-points/frontend.md) are not part of the product data and have been stored in the Redux store, we now need to retrieve the data in order for our React component to re-render.
...
Now we change the implementation that already exists in `frontend/LoyaltyPoints/connector.js` from the [Loyalty Points Tutorial](../../../tutorials/starter/loyalty-points/frontend.md#creating-the-connector) to the following:
...
**We import our newly created `getLoyaltyPointsByProductId` selector and use it instead of the one that was used in the [Loyalty Points Tutorial](../../../tutorials/starter/loyalty-points/frontend.md#creating-the-connector).**
...
We need to pass this function as the first argument of `connect`. The HOC then returns another function that we can use to inject the newly created props into the React component. You can see this inside the [Loyalty Points Tutorial](../../../tutorials/starter/loyalty-points/frontend.md).

**Now our component automatically receives the correct loyalty points when they are fetched, as seen in the guide on [fetching and processing data](data-fetching-and-processing.md)**
